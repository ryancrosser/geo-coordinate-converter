...
p.Geodetic = function pGeodeticConstructor(lat, lng) { //Represents a decimal degrees formatted geodetic coordinate
    if (isNaN(lat)) {
...
p.geodeticFromDM = function pGeodeticFromDM(latDeg, latMin, lngDeg, lngMin) {
    if (isNaN(latDeg)) {
...
p.geodeticFromDMS = function pGeodeticFromDMS(latDeg, latMin, latSec, lngDeg, lngMin, lngSec) {
    if (isNaN(latMin)) {
        throw 'Latitude minute is not a number';
    }
    if (isNaN(latSec)) {
...
p.Geodetic.prototype.toString = function pGeodeticToString(sigDig) {
    ...
    return (this.lat < 0 ? 'S' : 'N') + Math.abs(this.lat)
            .toFixed(sigDig) + (this.lng < 0 ? ' W' : ' E') + Math.abs(this.lng).toFixed(sigDig);
};
...
p.convertGeodeticToUTM = function pConvertGeodeticToUTM(geo) {
    ...
    if (geo instanceof p.Geodetic) {
        ...
        utmZone = getZoneNumber(geo.lat, geo.lng);
...
p.getGeodeticBoxArea = function pGetGeodeticBoxArea(cornerOne, cornerTwo) {
    ...
    if (cornerOne instanceof p.Geodetic && cornerTwo instanceof p.Geodetic) {
        result = (Math.PI / 180) * Math.pow(wgs84a, 2) * (Math.abs(Math.sin(cornerOne.lat * degToRad) - Math.sin(cornerTwo.lat * degToRad))) * Math.abs(cornerOne.lng - cornerTwo.lng);
    }
...
function getZoneNumber(lat, lng) {
